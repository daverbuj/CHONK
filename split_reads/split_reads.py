...
for Aln in bam.fetch(reference=chrom):
	...
	if len(s_alns)>0:
		...
		# 1. sort the alignments based on the qpos 

		# 2. from left-to-right on the query

		# check in pairs if the alignments
		# support a deletion or duplication
		# for (ind,ele) in enumerate(list):
			# if ind = len(list): break 
			# left = list[ind]
			# right = list[ind+1] 

		# same chrom, same strand, and not overlapping positions


		# chrom and strand have to be the same
		# dels: left alignment right pos < right alignment left pos
		# dups: left alignment left pos > right alignment right pos

		# breakpoints
		# dels: left alignment right pos - right alignment left pos
		# dups: right alignment left pos - left alignment right pos
