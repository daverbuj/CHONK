...
# inputs are bam file, mask file, fasta file, genome name, chromosome, and output name
def metadataExtraction(Args):
	...
	for chromosome in Args.r:
		chrom += (Backend.check_chrom(chromosome,AlnFile.chrom_flag),)
	...
	cwd = Args.o
	chrom_bed = "{}/{}.{}.CHONKdel.bed".format(cwd, genome, chrom_label)
	with open(chrom_bed, 'wt') as cbed_file:
		cbed_writer = csv.writer(cbed_file, delimiter='\t')
		for chromo in chrom:
			chrom_length = AlnFile.bam.lengths[int(chromo)-1]
			cbed_writer.writerow([chromo, 0, chrom_length])
	cbed_file.close()
	...
	for chromosome in chrom:
		chromosome_metadata[chromosome] = {'meta':()}
		mpd = []
		read_length_sum = 0
		read_count = 0
		bp_count = 0
		...
		with open(filtered_bed) as tsv:
			for mask_loc in csv.reader(tsv, dialect="excel-tab"):
				...
				mask_end = mask_loc[2]
				if chromosome == mask_chrom:
					for Aln in AlnFile.bam.fetch(reference = chromosome, start = int(mask_start), end = int(mask_end)):
						...
						if int(mask_start) <= midpoint <= int(mask_end):
							if read_count > 5e6:
			...
			chr_metadata = (chr_doc, avg_length, med_mpd, mad)
			chromosome_metadata[chromosome]['meta'] += chr_metadata
	...
	for chromo in chrom:
		...
		for win_size in [25, 1000]:
			chromosome_metadata[chromo][win_size] = {}
			num_wins = 10000
			grouped_bedfile = "{}/{}.{}.CHONKdel.masked.filtered.window{}.gc.grouped.bed".format(cwd, genome, chrom_label, win_size) 
			...
			with open(selected_bed) as tsv:
				for window in csv.reader(tsv, dialect="excel-tab"):
					...
					if chromo == window[0]:
						...
						win_end = window[2]
						win_gc = window[3]
						for Aln in AlnFile.bam.fetch(reference = chromo, start = int(win_start), end = int(win_end)):
							if Aln.is_unmapped: continue
							midpoint = ((Aln.reference_end - Aln.reference_start) / 2) + Aln.reference_start
							if int(win_start) <= midpoint <= int(win_end):
								count += 1
						aln_count[win_gc].append(count)
			chromosome_metadata[win_size] = {}
			for binid in aln_count:
				mean_count = np.mean(aln_count[binid])
				std_count = np.std(aln_count[binid])
				chromosome_metadata[chromo][win_size][binid] = (mean_count, std_count)

	# Adding chromosomal DOC, MPD, MAD data and gc bin data to a tuple to output one object as json file 
	# metadata = ((genome, chrom, avg_length, chr_doc, med_mpd, mad), {})
	metadata = ((genome, chrom, bamfile, fasta), chromosome_metadata)
	

	# Export chrom_metadata and gc_metadata as JSON file
	meta_json = '{}/{}.{}.metadata.chonk.json'.format(Args.o, genome, chrom_label)
	...
	chonkdel_cwd = "{}/{}.{}.CHONKdel*".format(cwd, genome, chrom_label)
